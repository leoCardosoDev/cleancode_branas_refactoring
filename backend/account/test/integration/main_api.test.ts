...
test("Deve obter um passageiro", async () => {
  ...
  const response = await axios.get(`http://localhost:3002/passengers/${outputCreatePassender.passengerId}`);
  const output = response.data;
...
test("Deve obter um motorista", async () => {
  ...
  const responseCreateDriver = await axios.post("http://localhost:3002/drivers", input);
  const outputCreateDriver = responseCreateDriver.data;
  const response = await axios.get(`http://localhost:3002/drivers/${outputCreateDriver.driverId}`);
  const output = response.data;
  expect(output.name).toBe("John Doe");
  expect(output.email).toBe("john.doe@test.com");
  expect(output.document).toBe("834.326.160-74");
